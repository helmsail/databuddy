...
public class MySqlSqlDialect implements SqlDialect {
	...
	public String listColumnsSql(String schema, String table) {
		return "SELECT column_name, column_type, is_nullable, column_comment FROM information_schema.columns "
				+ "WHERE table_schema = " + schemaRef(schema) + " AND table_name = " + literal(table)
	...
	/** schema 未指定时用当前连接库(DATABASE()):配置粒度直接到库,URL 已带库名 */
	private String schemaRef(String schema) {
		return schema == null || schema.isBlank() ? "DATABASE()" : literal(schema);
	}
