 ...
 * 当前拓扑:入口 → 意图识别 →(chat)终点 /(data_analysis)知识召回 → 查询增强 → Schema 召回 → 终点(数据链后续节点接入时顺延)
 */
@Configuration
