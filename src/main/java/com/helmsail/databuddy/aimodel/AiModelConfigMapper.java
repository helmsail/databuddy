...
public interface AiModelConfigMapper {
	...
	/** 全部配置(同类型激活在前,再按 id) */
	@Select("SELECT id, model_type, model_name, base_url, api_key, temperature, max_tokens, top_p, is_active, create_time, update_time FROM ai_model_config ORDER BY model_type, is_active DESC, id")
	List<AiModelConfig> selectAll();

	/** 按 id 查;不存在返回 null */
	@Select("SELECT id, model_type, model_name, base_url, api_key, temperature, max_tokens, top_p, is_active, create_time, update_time FROM ai_model_config WHERE id = #{id}")
	AiModelConfig selectById(@Param("id") Long id);

	/** 某类型的激活行(唯一索引保证至多一个);无返回 null */
	@Select("SELECT id, model_type, model_name, base_url, api_key, temperature, max_tokens, top_p, is_active, create_time, update_time FROM ai_model_config WHERE model_type = #{type} AND is_active = 1")
	...
	/** 失活:该类型激活位置 NULL(按类型操作,无激活行时影响 0 行,实同空转) */
	@Update("UPDATE ai_model_config SET is_active = NULL WHERE model_type = #{type} AND is_active = 1")
	void deactivate(@Param("type") AiModelType type);
